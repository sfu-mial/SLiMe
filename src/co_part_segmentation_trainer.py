...
class CoSegmenterTrainer(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.config.self_attention_loss_coef > 0:
            ...
            if sd_self_attention_maps is not None:
                small_sd_cross_attention_maps2 = torch.nn.functional.interpolate(
                ...
                for i in range(len(self.config.parts_to_return)):
                    ...
                    loss2 = loss2 + torch.nn.functional.mse_loss(
    ...
    def get_patched_masks(
        self, image, patch_size, output_mask_size, num_patchs_per_side, threshold
    ):
        crops_coords = get_crops_coords(
            image.shape[2:], patch_size, num_patchs_per_side
        )

        final_attention_map = torch.zeros(
            len(self.config.parts_to_return), output_mask_size, output_mask_size
        ).to(self.stable_diffusion.device)
        aux_attention_map = (
            torch.zeros(
        ...
        for crop_coord in crops_coords:
            ...
            if self.config.argmax_ca_before_sa:
                sd_cross_attention_maps2 = torch.where(
                    sd_cross_attention_maps2
            ...
            if not self.config.not_use_self_attention:
                sd_self_attention_maps = torch.nn.functional.interpolate(
                ...
                sd_self_attention_maps = torch.where(
            ...
            sd_cross_attention_maps2 = sd_cross_attention_maps2.flatten(
                1, 2
            ).detach()  # len(self.config.checkpoint_dir) , 64x64
            ...
            if len(passed_indices) > 0:
                ...
                for idx, mask_id in enumerate(passed_indices):
                    if not self.config.not_use_self_attention:
                        ...
                        final_attention_map[
                            mask_id, mask_y_start:mask_y_end, mask_x_start:mask_x_end
        ...
        return final_mask.cpu()

    def zoom_and_mask(self, image, output_mask_size, threshold, batch_idx):
        final_attention_map = torch.zeros(
            len(self.config.parts_to_return), output_mask_size, output_mask_size
        )
        with torch.no_grad():
        ...
        self_attention_map = torch.nn.functional.interpolate(
            self_attention_map[None, ...],
        ...
        if len(passed_indices) > 0:
            ...
            for idx, mask_id in enumerate(passed_indices):
                if not self.config.not_use_self_attention:
                    avg_self_attention_map = (
                        passed_attention_maps[idx][..., None, None] * self_attention_map
                    ).mean(dim=0)
                    avg_self_attention_map_min = avg_self_attention_map.min()
                    avg_self_attention_map_max = avg_self_attention_map.max()
                    ...
                    final_attention_map[mask_id] += torch.nn.functional.interpolate(
        ...
        final_mask = torch.zeros(output_mask_size, output_mask_size)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.config.log_images:
            ...
            self.logger.experiment.add_image(
                "val predicted mask", predicted_mask_grid, batch_idx
            )
    ...
    def on_validation_epoch_end(self):
        if self.config.val_data_ids is not None:
            print("save weight on validation end")
            ...
            text_embedding = torch.cat(
                [
                    ...
                    *list(
                    ...
                    text_embedding[:, 1 + len(self.embeddings_to_optimize) :],
                ],
                dim=1,
            )
            # text_embedding = torch.cat(
