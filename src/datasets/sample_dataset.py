...
class SampleDataset(Dataset):
    def __init__(self, image_dir, segmentation_dir, location1, location2, num_crops=1, crop_ratio=0.93, train=True, mask_size=256):
        ...
        if segmentation_dir is not None:
            ...
            self.segmentation = transforms.functional.resize(segmentation,
    ...
    def __getitem__(self, idx):
        ...
        if self.train:
            ...
            if self.segmentation is not None:
                cropped_segmentation = self.segmentation.crop((x, y, x + 512, y + 512))
                cropped_segmentation = transforms.functional.resize(cropped_segmentation, size=(self.mask_size, self.mask_size),
                                                            interpolation=transforms.InterpolationMode.NEAREST)
                kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (10, 10))
                cropped_segmentation = cv2.erode(cv2.dilate(np.array(cropped_segmentation), kernel, iterations=1), kernel, iterations=1)
