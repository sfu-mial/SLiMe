...
class PascalVOCPartDataset(Dataset):
    def __init__(
        ...
        if self.keep_aspect_ratio:
            ...
            self.test_transform_1 = A.Compose([A.Resize(512, 512), ToTensorV2()])
        else:
            self.train_transform_1 = A.Compose(
                [
                    A.Resize(512, 512),
    ...
    def preprocess_dataset(
        ...
        for line in progress_bar:
            ...
            if is_class_data == "-1":
                continue
            ann_file_path, img_file_path = get_file_dirs(
                f"{ann_file_id}.mat",
                ann_file_base_dir=ann_file_base_dir,
            ...
            if remove_overlapping_objects:
                ...
                if len(object_bbox_masks) > 0:
                    ...
                    object_bbox_masks = np.reshape(
                        object_bbox_masks, (object_bbox_masks.shape[0], -1)
                    )
                    intersection = object_bbox_masks @ object_bbox_masks.T
                    a = np.stack(
                        [
                            np.sum(object_bbox_masks, axis=1),
                            np.ones(object_bbox_masks.shape[0]),
                        ],
                        axis=1,
                    )
                    b = np.concatenate(
                        [
                            np.ones((1, object_bbox_masks.shape[0])),
                            np.sum(object_bbox_masks, axis=1)[None, ...],
                        ],
                        axis=0,
                    )
                    union = a @ b
            ...
            if self.single_object:
                ...
                for object_id in range(num_objects):
                    ...
                    for part_id in range(num_parts):
                        ...
                        aux_data = aux_part_data.get(
                            part_mapping[object_name][part_name], []
                        )
                    ...
                    if adjust_bounding_box:
                        ...
                        if width < height:
                            x_min, x_max = adjust_bbox_coords(
                                height, width, x_min, x_max, m_w
                            )

                        elif width > height:
                            y_min, y_max = adjust_bbox_coords(
                                width, height, y_min, y_max, m_h
                            )
                ...
                if not includes_part:
                    continue
                x_min, x_max, y_min, y_max = get_bbox_data(
                    np.where(whole_objects_mask > 0, 1, 0)
                )
                self.data.append(
                    {
                        "file_name": f"{ann_file_id}.mat",
    ...
    def __getitem__(self, idx):
        data = self.data[idx]
        if self.single_object:
            file_name, object_id, part_data, bbox = (
                data["file_name"],
                data["object_id"],
                data["part_data"],
                data["bbox"],
        ...
        if self.blur_background:
            ...
            image = Image.fromarray(
                np.where(
                    whole_mask[..., None] > 0, np.array(image), np.array(blurred_image)
                ).astype(np.uint8)
            )
        if self.fill_background_with_black:
            image = Image.fromarray(
                (np.array(image) * np.where(whole_mask[..., None] > 0, 1, 0)).astype(
                    np.uint8
                )
            )
        ...
        if self.train:
            ...
            while not mask_is_included:
                x_start, x_end, y_start, y_end = get_random_crop_coordinates(
                    (self.min_crop_ratio, 1), image.shape[1], image.shape[0]
                )
                ...
                #     mask_is_included = True
            # else:
            #     x_start, x_end, y_start, y_end = 0, 512, 0, 512
            #     aux_mask = mask
...
class PascalVOCPartDataModule(pl.LightningDataModule):
    ...
    def val_dataloader(self):
        # if self.val_data_ids is not None:
        return DataLoader(
