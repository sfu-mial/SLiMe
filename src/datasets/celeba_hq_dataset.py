...
class CelebaHQDataset(Dataset):
    ...
    def preprocess(
        ...
        for file_name in tqdm(files_names):
            ...
            masks_paths = glob(
                os.path.join(
                    masks_dir, str(mask_folder_idx), f"{file_name.zfill(5)}_*.png"
                )
            )
            part_data_paths = {}
            ...
            if len(data_ids) > 0 and len(self.images_paths) == max(data_ids) + 1:
                break
        if len(data_ids) > 0:
            aux_images_paths = []
            aux_masks_paths = []
    ...
    def __getitem__(self, idx):
        ...
        for idx, part_name in enumerate(self.parts_to_return):
            ...
            if part_name in masks_paths:
                for path in masks_paths[part_name]:
                    mask = np.where(
                        cv2.resize(
                            cv2.imread(path), (512, 512), interpolation=cv2.INTER_LINEAR
                        )[:, :, 0]
                        / 255
                        > 0.5,
                        1,
        ...
        if self.return_whole:
            final_mask = np.where(final_mask > 0, 1.0, 0.0)
        if self.train:
...
class CelebaHQDataModule(pl.LightningDataModule):
    ...
    def train_dataloader(self):
        return DataLoader(
            self.train_dataset, batch_size=self.batch_size, num_workers=3, shuffle=True
        )

    def val_dataloader(self):
        return DataLoader(
            self.val_dataset, batch_size=self.batch_size, num_workers=3, shuffle=False
        )

    def test_dataloader(self):
        return DataLoader(
            self.test_dataset, batch_size=self.batch_size, num_workers=3, shuffle=False
        )
